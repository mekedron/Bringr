...
/// A bundled value for both knobs (`enabled` + `duration`) is what `RadialMenuController`
/// resolves once per summon, mirroring how the other settings are read fresh so a
...
///
/// Visual cue: a progress arc on the slice's *own outer border*, parallel to the
/// cursor-centred ring in Bringr-93j.103 — same fill pattern, different anchor (the
/// slice you're aiming at, not the cursor that's aiming).
enum DwellActivation {
    ...
    /// The bundled (enabled, duration) the controller reads once per summon, like the
    /// other read-fresh settings. Resolved through `current(from:)` so a Preferences
    /// change takes effect on the next open without a relaunch.
    ...
    /// Resolve the persisted (enabled, duration) for the next summon.
    static func current(from defaults: UserDefaults = .standard) -> Config {
        Config(enabled: isEnabled(from: defaults), duration: duration(from: defaults))
    }
}
