...
/// Hold-to-activate / dwell (Bringr-93j.105), split into its own extension so
/// `RadialMenuController.swift` stays under the file-length budget (mirroring
/// `+Cursor` and `+Keyboard`). Resting the cursor on a slice for the configured
/// duration auto-commits it through the same click path the mouse and keyboard
/// already use.
///
/// The visual cue is an arc on the dwelt slice's outer border, drawn by
...
extension RadialMenuController {
    ...
    /// is fine). A `.none` (dead zone or off the wheel) clears the cue.
    ...
    /// Cleared first so the cue disappears the instant the commit runs.
